...
- **RORE**: the Oven coordination protocol. SUN currently implements its fast
  dispatch, lease, receipt, and heartbeat state with Redis Streams and TTL keys.
...
`rore.worker.v1` only through the scoped, authenticated `/autos_worker` HTTP
channel.
